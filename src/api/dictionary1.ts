...
const logger = Log4js.getLogger('dictionary1');

router.prefix('/api/dictionary1');

//获取指定的类型
router.get('/:id', async (ctx, next) => {
    const id = ctx.params.id;

    let dictionary1 = await ctx.models.dictionary1.where({id: id}).fetch();
...
router.get('/', async (ctx, next) => {
    ...
    ctx.body.total = await ctx.models.dictionary1.query(where).count();
    ctx.body.data = await ctx.models.dictionary1.query(where).orderBy(sort, order).fetchPage({limit, offset});
...
router.post('/', async (ctx, next) => {
    ...
    const result = await ctx.models.dictionary1.forge().save(fd);
...
router.put('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.dictionary1.forge({id}).save(fd);
...
router.delete('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.dictionary1.forge({id}).save({deleted: true, updated_by, updated_by_who}, {patch: true});
