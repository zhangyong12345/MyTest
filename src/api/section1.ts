...
const logger = Log4js.getLogger('section1');

router.prefix('/api/section1');

//获取指定的类型
router.get('/:id', async (ctx, next) => {
    const id = ctx.params.id;

    let section1 = await ctx.models.section1.where({id: id}).fetch();
...
router.get('/', async (ctx, next) => {
    ...
    const where = function(qb) {
        ...
        if (!!division) qb.andWhere('code', 'LIKE', `%${division}%`); 
        if (!!group) qb.andWhere('name', 'LIKE', `%${group}%`); 
    }
    
    ctx.body = {'code':0, 'msg':'success'};
    ctx.body.total = await ctx.models.section1.query(where).count();
    ctx.body.data = await ctx.models.section1.query(where).orderBy(sort, order).fetchPage({limit, offset});
...
router.post('/', async (ctx, next) => {
    ...
    const result = await ctx.models.section1.forge().save(fd);
...
router.put('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.section1.forge({id}).save(fd);
...
router.delete('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.section1.forge({id}).save({deleted: true, updated_by, updated_by_who}, {patch: true});
