...
const logger = Log4js.getLogger('exception-enterprise');

router.prefix('/api/exceptionEnterprises');

//获取指定的企业信息
router.get('/:id', async (ctx, next) => {
    const id = ctx.params.id;

    let exception_enterprise = await ctx.models.exception_enterprise.where({id: id}).fetch();

    if (!exception_enterprise) {
        ctx.response.status = 404;
        ctx.response.statusString = '字典条目不存在！';
        return;
    }
    
    ctx.body = exception_enterprise;
});
...
router.get('/', async (ctx, next) => {
    ...
    ctx.body.total = await ctx.models.exception_enterprise.query(where).count();
    ctx.body.data = await ctx.models.exception_enterprise.query(where).orderBy(sort, order).fetchPage({limit, offset});
});
...
router.post('/', async (ctx, next) => {
    ...
    const result = await ctx.models.exception_enterprise.forge().save(fd);
...
router.put('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.exception_enterprise.forge({id}).save(fd);
...
router.delete('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.exception_enterprise.forge({id}).save({deleted: true, updated_by, updated_by_who}, {patch: true});
