...
const logger = Log4js.getLogger('org-enterprises');

router.prefix('/api/orgEnterprises');

//获取指定的企业信息
router.get('/:id', async (ctx, next) => {
    const id = ctx.params.id;

    let org_enterprise = await ctx.models.org_enterprise.where({id: id}).fetch();
...
router.get('/', async (ctx, next) => {
    ...
    ctx.body.total = await ctx.models.org_enterprise.query(where).count();
    ctx.body.data = await ctx.models.org_enterprise.query(where).orderBy(sort, order).fetchPage({limit, offset});
});
...
router.post('/', async (ctx, next) => {
    ...
    const result = await ctx.models.org_enterprise.forge().save(fd);
...
router.put('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.org_enterprise.forge({id}).save(fd);
...
router.delete('/:id', async (ctx, next) => {
    ...
    const result = await ctx.models.org_enterprise.forge({id}).save({deleted: true, updated_by, updated_by_who}, {patch: true});
